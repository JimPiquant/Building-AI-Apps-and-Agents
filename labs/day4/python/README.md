...
| [`roles.py`](roles.py) | A/B/C | Shared: Planner/Retriever/Critic agent factories, their structured-output models (`Plan`, `Evidence`, `RetrievalResult`, `Answer`, `CriticVerdict`), the local `search_docs` tool, and `load_golden_set()` |
...
| [`part_b_custom_graph.py`](part_b_custom_graph.py) | B | `WorkflowBuilder` + conditional edge + required budget guardrail (`revision_gate`, `finalize`) |
| [`tests/test_part_b_guardrail.py`](tests/test_part_b_guardrail.py) | B | Isolation tests for the guardrail's `compute_next_step()` decision logic — no live Foundry call |
