// Build Day 4 decks from the Markdown plans in slides/day4/.
...
const SLIDES_DIR = path.join(ROOT, "slides", "day4");
const OUT_DIR = path.join(ROOT, "decks", "day4");

const MODULE_FILES = [
  "module-1-agents-vs-workflows.md",
  "module-2-orchestration-patterns.md",
  "module-3-workflow-fundamentals.md",
  "module-4-multi-agent-memory.md",
  "module-5-evaluation.md",
  "module-6-failure-modes.md",
  "module-7-lab-kickoff.md",
];
...
function parseSlides(body) {
  ...
  return matches.map((match, index) => {
    ...
    return {
      ...
      image: layout === "image" || layout === "list-image" ? parseImage(visible) : null,
      demoTime,
      demoReference,
...
function parseModule(fileName) {
  ...
  for (const slide of slides) {
    ...
    if (slide.layout === "image" || slide.layout === "list-image") {
      if (!slide.image) {
    ...
    if (slide.layout === "list-image" && slide.items.length === 0) {
      throw new Error(`list-image slide has no bullets: ${fileName} / ${slide.title}`);
    }
  }
...
// Scales an image to fit within an explicit box, preserving its true aspect
// ratio (read straight from the PNG's own header), and centers it both
// horizontally and vertically within that box. Shared by `layout: image`
// (the full content-width box) and `layout: list-image` (a narrower
// right-hand box paired with wrapped bullets on the left) — this is the
// right call whenever the *exact* source artifact matters more than
// matching our deck's house style. The deck's standard source footer
// (always present via addSourceFooter) supplies the "reproduced from"
// credit, so no separate caption is added.
function renderImageBoxed(slide, image, box) {
...
function renderImage(slide, image) {
  const top = 1.18;
  const bottom = 5.15;
  renderImageBoxed(slide, image, { x: 0.4, y: top, w: 9.2, h: bottom - top });
}

// Bullets on the left, an image on the right, side by side — the fix for
// an image whose native aspect ratio (e.g. a tall/portrait diagram) would
// otherwise waste most of a full-width `layout: image` slide. The left
// column gets just enough width for comfortable wrapping; the right
// column gets the remainder, and renderImageBoxed scales/centers whatever
// aspect ratio the image actually has within it — no overlap is possible
// since the two columns never share x-coordinates.
function renderListImage(slide, items, image) {
  const top = 1.18;
  const bottom = 5.15;
  const gap = 0.3;
  const leftW = 4.4;
  const rightX = 0.4 + leftW + gap;
  const rightW = 9.2 - leftW - gap;
  renderList(slide, items, { x: 0.4, y: top, w: leftW, h: bottom - top });
  renderImageBoxed(slide, image, { x: rightX, y: top, w: rightW, h: bottom - top });
}
...
function renderSlide(pres, module, spec) {
  ...
  if (spec.layout === "title") {
    ...
    else if (spec.layout === "list-image") renderListImage(slide, spec.items, spec.image);
...
async function buildModule(module) {
  ...
  pres.subject = "Day 4 — Multi-Agent Patterns + Evaluation Anchor";
...
async function main() {
  ...
  console.log("Building Day 4 decks from Markdown…");
  const outputs = [];
