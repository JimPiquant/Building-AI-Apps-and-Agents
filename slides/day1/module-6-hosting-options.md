...
A **Prompt agent** is authored entirely as configuration — instructions, model, tools. Author in the Foundry portal (portal-first), via the SDK / REST (code-first, for CI/CD), or via a declarative YAML definition. Either way, **Foundry runs it**. No application code to maintain, no compute to pay for, no containers to patch.
