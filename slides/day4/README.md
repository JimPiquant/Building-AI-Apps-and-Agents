...
- **Module 3's time extended** from 30m to 55m to fit its content without cutting scope — the original workflow-fundamentals content (30m→45m), plus three additions closing gaps against the official [Workflow capabilities](https://learn.microsoft.com/en-us/agent-framework/workflows/) index: **visualization** (`WorkflowViz`/Mermaid/DOT), **workflow-level observability** (OTel spans, `edge_group.delivery_status` for debugging conditional edges), and **declarative workflows** (YAML) — now three real content slides (overview, when-to-use table, Python YAML skeleton), not just a mention. The required Parts A-C of the lab stay `WorkflowBuilder`-based; an optional Part D, added later, gives declarative workflows a hands-on counterpart.
- The lab's implementation shape is **decided (2026-08-31)**, not the single workflow v0.7 originally described. It shipped as a 4-part progression: **Part A** (`SequentialBuilder`, no correction — demonstrates the limitation), **Part B1** (a custom `WorkflowBuilder` graph with a conditional Critic→Planner edge — the fix), **Part B2** (a required, test-driven budget guardrail on that loop, since a conditional edge has no built-in `max_iterations`), **Part C** (`GroupChatBuilder` with an `orchestrator_agent` playing the Critic's role — an alternative fix, compared against Part B on the same golden set). `labs/day4/` (the actual lab code) is authored and current, and Module 7's kickoff slides reflect the A/B1/B2/C structure. An optional, provided-complete **Part D** was added afterward — it loads a declarative YAML workflow via `WorkflowFactory`, separate from the graded A-C progression and not part of `evaluate.py`'s comparison.
